...
resume_extractor = Agent(
    ...
    "You will return the results in a structured format. \n filename: [name of file], github link: [extracted github link]\
        example github link - https://github.com/[username] extract only this link from the resume. \
            don't include https://github.com/[username]/[repo] links",
...
Github_Analyser = Agent(
    ...
    goal="You are a github analysis agent.You will get the github link by resume extractor in the format filename: [name of file], github link: [github link] \
        You will analyze the github links extracted from the resumes and provide insights using ScrapeWebsite tool.\" \
            use https://github-readme-stats.vercel.app/api?username=[github_username] for github stats to fetch prs,contributions. Extract the data exactly \
            also check for the pull requests, issues, and contributions in the github profile. and make sure to include it everytime in report \
    ...
    backstory="You are an expert in analyzing github profiles. You will analyze the github link extracted from the resume and provide insights.",
    llm=gemini_llm,
    tools=[scrapper],
    verbose=True,
    memory=True
...
github_task=Task(
    ...
    description=("Analyze the github links extracted from the resumes and provide insights.\
                 use https://github-readme-stats.vercel.app/api?username=[github_username] for github stats to fetch prs contributions\
                 github username is the username part of the github link after github.com/[username]. "),
    ...
    "write the output to file only if that user has Total PRs:(equal to or greater than given {threshold} value. if its equal include it)\n \
    if not found, then don't put that in file'.\n"),
...
# date = "2025-05-31"  # Example date, replace with user input if needed
# threshold = 0  # Example threshold, replace with user input if needed
